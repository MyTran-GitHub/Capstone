...
compute_results <- function(focal.years, outcome, ci_type = "two") {
  ...
  # (2006, 2008, 2010 correspond to the 3 windows)
  ...
  for (w_name in names(windows)) {
    ...
    for (outcome in outcomes) {
      ...
      fire_label <- switch(outcome,
                           fire_all = "All fires",
                           fire_90  = "Class 2 ^`^s5 fires",
                           fire_95  = "Class 3 ^`^s5 fires")
      plot_list[[k]] <-
...
# 1  ^o ^c  Two-sided CI (3 ^w3)
grid_two <- build_grid(ci_type = "two")

ggsave(file.path(resDir, "results",
                 "conifer_all_windows_two_sided.jpeg"),
       grid_two,
       width = 15,
       height = 12,
       units = "in")

# 2  ^o ^c  One-sided CI (3 ^w3)
grid_one <- build_grid(ci_type = "one")

ggsave(file.path(resDir, "results",
                 "conifer_all_windows_one_sided.jpeg"),
       grid_one,
       width = 15,
       height = 12,
       units = "in")
