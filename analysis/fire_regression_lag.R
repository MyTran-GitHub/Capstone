...
required_packages <- c("grid", "pBrackets", "gridExtra", "Hmisc")
...
res <- list()
k = 1  
#run_sensitivity <- FALSE # Set to TRUE to run sensitivity analyses

# MAIN ANALYSIS (robust: loop over cohort years and lags, dynamic year extraction)
cohort_years <- c(2006, 2008, 2010)
all.lags <- 1:9

for (outcome in c("fire_all", "fire_90", "fire_95")) {
  for (biome in "conifer") {
    # Read and combine all cohort years and lags
    data.raw <- do.call(rbind, lapply(cohort_years, function(cy) {
      do.call(rbind, lapply(all.lags, function(ll) {
        fpath <- file.path(outDir, "rev_result_low", as.character(cy), paste0(biome, "_lag", ll, ".csv"))
    ...
    # Dynamically extract all available end years from the data
    all.end.years <- sort(unique(data.reg$end.year))
    ...
    ggsave(file.path(resultDir, paste("cmp", biome, fire_type, ".jpeg", sep="_")),
           cmp.plot,
           width = 11,
           height = 8.5, unit = "in")
...
ggsave(file.path(resultDir, "results", paste0("res_combined_rev_linear.jpeg")), 
       res_combined, 
       width = 14 / 1.6*2,
       height = 8.5 / 1.6*3,
       units = "in")
...
if (run_sensitivity) {
  ...
  ggsave(file.path(resultDir, "results", paste0("res_combined_rev_linear_90.jpeg")), 
        res_combined, 
        width = 14 / 1.6*2,
        height = 8.5 / 1.6*3,
        units = "in")
  ...
  ggsave(file.path(resultDir, "results", paste0("res_combined_mtbs_linear.jpeg")), 
        res_combined, 
        width = 14 / 1.6*2,
        height = 8.5 / 1.6*3,
        units = "in")
  ...
  ggsave(file.path(resultDir, "results", paste0("res_combined_prescribed_linear.jpeg")), 
        res_combined, 
        width = 14 / 1.6*3,
        height = 8.5 / 1.6*3,
        units = "in")
  ...
  res_combined <- grid.arrange(res[[1]], res[[2]], res[[3]], res[[4]], 
                              ...
                              nrow = 3)
  yearplus1 <- (years + 1)
  ggsave(file.path(resultDir, "results", paste0("res_combined_rev_linear_", yearplus1, ".jpeg")), 
        res_combined, 
        width = 14 / 1.6*2,
        height = 8.5 / 1.6*3,
        units = "in")
  ...
  ggsave(file.path(resultDir, "results", paste0("sens_combined_rev_linear.jpeg")), 
        res_combined, 
        width = 14 / 1.6*2,
        height = 8.5 / 1.6*3,
        units = "in")
...
# sensitivity on e-values
  ...
  ggsave(file.path(resultDir, "results", paste0(noconf, "_combined_rev_linear.jpeg")), 
        res_combined, 
        width = 14 / 1.6*2,
        height = 8.5 / 1.6*3,
        units = "in")
